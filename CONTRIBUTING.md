...
- In the npm package's Trusted Publishing settings, bind GitHub Actions to owner `guillemservera`, repository `vue-drawer`, workflow `release.yml` and environment `npm`. Allow `npm stage publish` only, not direct `npm publish`.
